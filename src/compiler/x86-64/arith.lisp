...
(define-vop (smulhi)
  ...
  (:args (x :scs (signed-reg) :target eax)
         (y :scs (signed-reg signed-stack)))
  (:arg-types signed-num signed-num)
  (:temporary (:sc signed-reg :offset eax-offset :from (:argument 0))
              eax)
...
(define-vop (mulhi-shift)
  ...
  (:temporary (:sc unsigned-reg :offset eax-offset :from (:argument 0))
              eax)
  (:temporary (:sc unsigned-reg :offset edx-offset :from (:argument 1)
                   :to (:result 0) :target result) edx)
  (:temporary (:sc unsigned-reg :offset ecx-offset :from (:argument 2))
              ecx)
  (:results (result :scs (unsigned-reg)))
  (:result-types unsigned-num)
  (:generator 22
...
(define-vop (mulhi-shift/c)
  ...
  (:generator 21
    ...
    (inst mul eax y)
    (move result edx)
    (unless (zerop shift) (inst shr result (min 63 shift)))))
...
(define-vop (smulhi-shift/c/fx)
  ...
  (:args (x :scs (any-reg) :target eax)
         (y :scs (signed-reg signed-stack)))
  (:arg-types tagged-num signed-num (:constant word))
  (:info shift)
  (:temporary (:sc any-reg :offset eax-offset :from (:argument 0)) eax)
  (:temporary (:sc any-reg :offset edx-offset :from (:argument 1)
                   :to (:result 0) :target result) edx)
